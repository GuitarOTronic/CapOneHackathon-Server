...
class usersController extends Controller {
    ...
    userMinusFamily (userId) {
        return usersModel.one(userId)
            .then(user => {
                let userObject = user
                let match = user.user_type === 'parent' ? ['parent_id', user.id] : ['child_id', user.id]
                let userData = [allowancesModel.allMatchingWhere(...match), requestsModel.allMatchingWhere(...match), goals.allMatchingWhere('user_id', user.id)]
    ...
    one (req, res, next) {
        ...
        return this.userMinusFamily(givenId)
            .then(mainUser => {
                ...
                return Promise.all(family)
                    .then(allFamily => {
                        ...
                        let familyUserCalls = familyIds.map(id => this.userMinusFamily(id))
